...
export async function processDocumentInBrowser({
  ...
  for (let index = 0; index < pages.length; index++) {
    ...
    const renderedPage = await renderPdfPageToPng(page, { dpi: settings.ocrDpi });
...
async function resolvePageSource({
  ...
  const textSufficient = allPdfItems.length >= OCR_TEXT_MIN_ITEMS;
  const shouldTryOcr = requestedMode === 'ocr'
    || requestedMode === 'hybrid'
    || (requestedMode === 'auto' && !textSufficient);
  ...
  if (requestedMode === 'ocr') {
    return {
      items: ocrItems,
      allItems: ocrItems,
      ocrApplied,
      ocrWarnings: ocrWarnings.length ? ocrWarnings : (!ocrItems.length ? ['OCR solicitado, mas nenhum texto foi extraido.'] : []),
      sourceMode: 'ocr',
      reason: ocrApplied ? 'ocr' : 'ocr-indisponivel',
    };
  }
  ...
  if (requestedMode === 'hybrid') {
    ...
    return {
      items: hybridItems,
      allItems: mergeOcrAndPdfItems({ pdfItems: allPdfItems, ocrItems }),
      ocrApplied,
      ocrWarnings,
      sourceMode: 'hybrid',
      reason: ocrApplied ? 'texto+ocr' : 'texto-sem-ocr',
    };
  }

  if (requestedMode === 'auto' && !textSufficient && ocrItems.length) {
    return {
      items: ocrItems,
      allItems: ocrItems,
      ocrApplied,
  ...
  if (requestedMode === 'auto' && !textSufficient && !ocrItems.length) {
    ocrWarnings.push('Pagina sem camada de texto suficiente e OCR indisponivel. A aba visual sera exportada, mas a revisao manual e necessaria.');
  }
  ...
  return {
    ...
    ocrWarnings,
    sourceMode: 'text',
    reason: textSufficient ? 'texto-pdf' : 'texto-insuficiente',
  };
}

async function tryRunOcr({ pageNumber, renderedPage, settings }) {
  const runtime = await ensureTesseractRuntime();
  if (!window.Tesseract || runtime.source === 'unavailable') {
    return {
      applied: false,
      items: [],
      warnings: ['OCR opcional indisponivel neste ambiente.'],
    };
  }

  try {
    const result = await window.Tesseract.recognize(renderedPage.dataUrl, settings.ocrLanguages || 'por+eng');
    ...
    return {
      ...
      items,
      warnings: items.length ? [] : ['OCR executado sem texto confiavel suficiente.'],
    };
  } catch (error) {
    return {
      applied: false,
      items: [],
      warnings: [`OCR falhou: ${error.message}`],
    };
  }
}
