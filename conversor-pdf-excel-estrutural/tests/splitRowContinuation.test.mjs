...
assert.equal(merged.currentSourceMatrix.length, 1);
...
function makeTable(pageNumber, matrix) {
  return {
    ...
    sourceRowMeta: matrix.map(() => ({ cellMeta: [], isProbablyHeader: false })),
    displayRowMeta: matrix.map(() => ({ cellMeta: [], isProbablyHeader: false })),
    headerRowIndex: -1,
  };
}
